---
name: "Geas"
type: spell
exists: true
state: known
tags:
  - spell
  - spell/enchantment
level: 5
school: enchantment
casting_time: "1 minute"
range: "60 feet"
components:
  - V
duration: "30 days"
concentration: false
effect: "Issue a Geas — a magical command — to a creature within range that it can understand. The creature must make a Wisdom save. On fail: it is compelled to carry out the service or refrain from an action or course of activity you describe. Any time it acts against the geas, it takes 5d10 psychic damage (once per day). The spell doesn't affect a creature that can't understand you, or is immune to charm. At higher levels: duration increases to 1 year at 7th, permanent at 9th."
available_to:
  - Bard
  - Cleric
  - Druid
  - Paladin
  - Wizard
description: "Level 5. 30 days. One creature: Wis save or magically compelled to follow a command. 5d10 psychic damage per day it disobeys."
---

Geas is long-duration compulsion for diplomatic and strategic control — 30 days of behavioral enforcement with 5d10 psychic per day for violations. Used to extract service, prevent betrayal, or compel information delivery. In Barovia, Geas is one of the few ways to create binding agreements with untrustworthy NPCs. At 7th or 9th level, it becomes year-long or permanent for truly critical situations.
