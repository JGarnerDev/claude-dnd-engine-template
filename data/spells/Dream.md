---
name: "Dream"
type: spell
exists: true
state: known
tags:
  - spell
  - spell/illusion
level: 5
school: illusion
casting_time: "1 minute"
range: "Special"
components:
  - V
  - S
  - M
material_component: "A handful of sand, a dab of ink, and a writing quill plucked from a sleeping bird"
duration: "8 hours"
concentration: false
effect: "Shape another creature's dreams. The target must be sleeping on the same plane of existence. You, or a willing creature you touch, enter a trance state and act as a messenger within the target's dreams. The dreamer can make a Wisdom save to notice the dream is artificial. Messenger role can heal the target (1d6+caster level HP) or torment them (no sleep, Wisdom save or gain no benefit from rest + 3d6 psychic damage on waking)."
available_to:
  - Bard
  - Warlock
  - Wizard
description: "Level 5. Enter target's dreams: deliver message, or torment (no rest benefit + 3d6 psychic). 8 hours."
---

Dream enables cross-plane messaging and psychological warfare — tormented enemies gain no benefit from long rests, making this a war-of-attrition spell against important targets who need rest-dependent abilities. As a message delivery mechanism, it bypasses many barriers since the target just needs to be asleep on the same plane. Using Dream offensively against a creature that doesn't sleep is ineffective; against humanoid lieutenants and NPCs, it is potent.
