...
describe('serializeState', () => {
  it('snapshots live parts, draining the track Set to an array', () => {
    const state = serializeState('siege', new Set(['world', 'factions']), 4, 120);
    expect(state).toEqual({ query: 'siege', tracks: ['world', 'factions'], zoomLevel: 4, scrollLeft: 120 });
  });
});
...
describe('applyDefaults', () => {
  ...
  it('fills missing fields and keeps valid ones', () => {
    expect(applyDefaults({ query: 'a' })).toEqual({ query: 'a', tracks: [], zoomLevel: 1, scrollLeft: 0 });
  });
  it('drops wrong-typed fields and non-string tracks', () => {
    const raw = { query: 7, tracks: ['ok', 3, null], zoomLevel: 'x', scrollLeft: NaN } as unknown;
